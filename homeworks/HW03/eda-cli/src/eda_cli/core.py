...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    high_cardinality_categoricals= [col.name for col in summary.columns if not col.is_numeric and
                                    col.unique/summary.n_rows >= 0.9]
    has_high_cardinality_categoricals = len(high_cardinality_categoricals) > 0
    ...
    if has_high_cardinality_categoricals:
        score -= 0.15 * len(high_cardinality_categoricals)
