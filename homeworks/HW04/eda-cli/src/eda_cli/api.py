...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool] = Field(
        ...,
        description="Словарь булевых флагов качества данных (включая новые из HW03)",
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор флагов качества из CSV",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Вариант A: вернуть полный набор флагов качества из compute_quality_flags.

    Принимает CSV-файл, запускает EDA-ядро (summarize_dataset + missing_table +
    compute_quality_flags) и возвращает ВСЕ булевы флаги, включая твои
    ...
    # Берём только булевы флаги (остальные, типа quality_score, списки и т.п. — отбрасываем)
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
